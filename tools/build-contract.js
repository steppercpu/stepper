...
/* The same keccak, for the browser.
 *
 * The launchpad predicts a chip address before the mint, which is
 * keccak(rlp(factory, nonce)), so the page needs the hash the toolchain
 * already has. It is emitted from that one file rather than reimplemented:
 * two keccaks in a project is two chances to have one of them wrong, and the
 * one that would be wrong is the one nothing tests. */
var KSRC = fs.readFileSync(path.join(__dirname, "netlist", "keccak.js"), "utf8")
  .replace(/module\.exports[^;]*;\s*$/m, "");

var KHEAD = [
  "// Generated by tools/build-contract.js from tools/netlist/keccak.js.",
  "// Do not edit by hand. It is the toolchain's own keccak, checked against",
  "// published vectors on every build, wrapped for the page.",
  "(function () {",
].join("\n");

var KTAIL = [
  "",
  "window.KECCAK = keccak256;",
  "})();",
  "",
].join("\n");

fs.writeFileSync(path.join(ROOT, "public", "scripts", "keccak.js"),
  KHEAD + "\n" + KSRC + KTAIL);
console.log("Wrote public/scripts/keccak.js");
