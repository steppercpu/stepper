...
function loadNetlist() {
  const file = path.join(ROOT, "public/scripts/st8-data.js");
  if (!fs.existsSync(file)) {
    die("public/scripts/st8-data.js is missing. Run `npm run silicon` first.");
  }
  const win = {};
...
const HELP = `
  node tools/run.js [file.asm] [options]
